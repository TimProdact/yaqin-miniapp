...
async function render() { nav(); try { if(currentTab==="discover") await renderDiscover(); if(currentTab==="matches") await renderMatches(); if(currentTab==="profile") await renderProfile(); } catch(error) { view.innerHTML=`<div class="empty"><h2>Не удалось загрузить</h2><p class="muted">${esc(error.message)}</p></div>`; } }
